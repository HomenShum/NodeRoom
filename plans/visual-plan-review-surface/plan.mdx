...
Use Visual Plans for NodeRoom work that is too risky, visual, or architecture-heavy to review as terminal Markdown. For executable agent work, the visual plan is a rendering of a structured plan object, not the authority itself.
...
flowchart TD
  ...
  Review -->|direction approved| Build[Implement code]
  Review -->|plan hash approved| Execute[Execute checked workflow]
  Review -->|questions| Revise[Revise plan]
  Build --> Verify[Tests / build / Playwright]
  Execute --> Actual[Planned-vs-actual report]
...
| Executable work | Structured plan object + canonical hash + planned-vs-actual report |
...
Structured source object when executable
Plan hash and planned-vs-actual path
```
...
- If executable, is the MDX/HTML clearly a rendering of structured data?
- Is approval tied to a canonical payload hash?
