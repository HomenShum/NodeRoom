...
flowchart TD
  ...
  OutputClass --> PublicOut[Room-visible proposals/traces/artifacts]
  OutputClass --> PrivateOut[Private proposals/traces/artifacts]
...
- Agent Artifacts inherit the maximum sensitivity of their inputs.
- Rendered MDX/HTML/React artifact views are permission-filtered renderings, not
  raw payload dumps.
...
6. Shipped backend: Agent Artifact `planHash` approval checks exist.
7. Add Agent Artifact rendering redaction.
8. Add privacy regression tests per shared surface.
...
- Agent Artifact rendering hides private refs from non-owners.
