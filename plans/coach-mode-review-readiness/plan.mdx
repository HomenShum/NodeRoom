...
flowchart TD
  ...
  Eval --> Feedback[CoachFeedbackArtifact]
...
| Area | Files |
|---|---|
| Planned dedicated UI | `src/ui/coach/*` (not yet present) |
| Evidence UI | `src/ui/artifacts/EvidenceCarouselArtifact.tsx`, `src/ui/panels/TraceSurface.tsx` |
| Planned backend | `convex/coach.ts`, `convex/coachEvaluator.ts` (not yet present) |
...
```text
coachSessions -> coachQuestions -> coachAnswers -> CoachFeedbackArtifact
                                     -> masteryTags
                                     -> missedEvidenceRefs
                                     -> followUpTasks
```
...
- CoachFeedbackArtifact links to evidence/source/cell.
