...
```mermaid
flowchart TD
  Input[Processed notebook blocks / sheet / source text] --> Normalize[Normalize: text, spans, hash]
...
| Native notebook target | `convex/notebookProcessing.ts`, `notebookDirtyEvents`, `notebookProcessingJobs`, processed notebook read model |
| Tests | `tests/passiveIntelligence.test.tsx`, `tests/roomActivityEvidenceAdapters.test.ts`, `tests/notebookProcessingTarget.test.ts` |
...
- permission or dirty-event attribution
...
6. For native notebooks, consume processed read-model rows, not raw
   ProseMirror snapshots or hot `elements["doc"]` mirrors.
