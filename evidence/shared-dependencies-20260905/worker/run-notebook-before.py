...
report=O/'notebook-browser-before'
...
with (report/'server.log').open('wb') as log:
    server=subprocess.Popen([node,str(C/'node_modules/vite/bin/vite.js'),'preview','--outDir',str(O/'baseline-preview/dist'),'--host','127.0.0.1','--port',str(port),'--strictPort'],cwd=C,env=env,stdout=log,stderr=subprocess.STDOUT,creationflags=subprocess.CREATE_NO_WINDOW)
    try:
