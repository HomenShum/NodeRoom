...
for file in sorted((O/'browser-replay').rglob('*.xlsx')):
 actual=parse_xlsx(file);oracle=json.loads(file.with_name(file.stem+'-reopened.json').read_text(encoding='utf-8'))
 ...
 files.append(actual)
lit=json.loads((O/'literal-keyboard/report.json').read_text(encoding='utf-8'))
for f in lit['files']:
...
generated=json.loads((W/'generated-final-manifest.json').read_text(encoding='utf-8'));bad=[]
...
old=json.loads((P/'E6f-noderoom-first-run/source-before.json').read_text(encoding='utf-8'));changes={}
