...
(function () {
  ...
  // ── shared pill ───────────────────────────────────────────────────────────
  function Pill({ tone = 'mute', children, icon }) {
    return React.createElement('span', { className: 'na-pill ' + tone },
      icon && Ico(icon), children);
  }
  ...
  function Capture({ ctx }) {
    ...
    const passiveIcon = t.passive === 'off' ? 'eyeOff' : 'eye';

    return React.createElement(React.Fragment, null,
      // mode indicator
      React.createElement('div', { className: 'na-modepill' },
        Ico(passiveIcon), 'Mode · ', React.createElement('b', null, passiveLabel)),

      // notebook
      React.createElement('div', { className: 'na-card accent na-note' },
        ...
        React.createElement('div', { className: 'na-note-foot' },
      ...
      // detected entities
      detected && t.passive !== 'off' && React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'na-kicker' }, 'Detected in this note'),
        React.createElement('div', { className: 'na-chips' },
          D.DETECTED.map((d, i) => React.createElement('button', {
            key: i, className: 'na-chip live',
      ...
      noticed && t.passive !== 'off' && React.createElement('button', {
        className: 'na-noticed', onClick: () => ctx.openSheet('plan'),
      },
        React.createElement('span', { className: 'ico' }, Ico('sparkles')),
        React.createElement('span', { className: 'na-noticed-copy' },
  ...
  function Inbox({ ctx }) {
    ...
    const card = (item) => React.createElement('button', {
      ...
      React.createElement('div', { className: 'na-card-head accent' },
        React.createElement('div', { className: 'na-card-title', style: { display: 'flex', gap: 10, alignItems: 'center' } },
    ...
    if (open.length === 0 && done.length === 0) return emptyState('inbox', 'Inbox is clear', 'Noteworthy findings, approvals, and coach prompts land here.');
    ...
    return React.createElement(React.Fragment, null,
      ...
      done.map((item) => React.createElement('div', {
        key: item.id, className: 'na-card', style: { opacity: .6 },
      },
        React.createElement('div', { className: 'na-card-head' },
          React.createElement('div', { className: 'na-card-title', style: { display: 'flex', gap: 10, alignItems: 'center' } },
  ...
  function Room({ ctx }) {
    ...
    return React.createElement(React.Fragment, null,
      ...
      React.createElement('div', { className: 'na-stats' },
        React.createElement('div', { className: 'na-stat' },
          React.createElement('b', null, ctx.resolvedCount), React.createElement('span', null, 'approvals waiting')),
        React.createElement('div', { className: 'na-stat' },
          React.createElement('b', { className: 'mono' }, ctx.version), React.createElement('span', null, 'sheet version')),
      ),
      ...
      React.createElement('div', { className: 'na-card' },
        React.createElement('div', { className: 'na-card-body', style: { paddingTop: 'var(--na-pad)' } },
          p.agents.map((a, i) => React.createElement('div', { key: i, className: 'na-row' },
            ...
            React.createElement(Pill, { tone: ctx.runState === 'running' ? 'accent' : 'mute' },
              ctx.runState === 'running' ? 'running' : 'idle'))))),

      React.createElement('div', { className: 'na-kicker' }, 'Recent findings'),
      React.createElement('div', { className: 'na-card' },
        React.createElement('div', { className: 'na-card-body', style: { paddingTop: 'var(--na-pad)' } },
          p.findings.map((f, i) => React.createElement('div', { key: i, className: 'na-row' },
      ...
      React.createElement('div', { className: 'na-card' },
        React.createElement('div', { className: 'na-card-body', style: { paddingTop: 'var(--na-pad)' } },
          p.people.map((person, i) => React.createElement('div', { key: i, className: 'na-row' },
  ...
  function Coach({ ctx }) {
    return React.createElement(React.Fragment, null,
      ...
      React.createElement('div', { className: 'na-card accent', 'data-accent-rule': 'priv' },
        React.createElement('div', { className: 'na-card-head accent' },
        ...
        React.createElement('div', { className: 'na-card-body accent' },
          ...
          React.createElement('button', { className: 'na-btn primary full', style: { marginTop: 12 }, onClick: () => ctx.openSheet('coach') },
            Ico('coach'), 'Start coaching'))),
  ...
  function emptyState(icon, title, sub) {
    return React.createElement('div', { className: 'na-empty' },
      React.createElement('div', { className: 'eico' }, Ico(icon)),
      React.createElement('strong', null, title),
      React.createElement('span', null, sub));
  }

  window.NAScreens = { Capture, Inbox, Room, Coach, Pill, riStyle };
})();
