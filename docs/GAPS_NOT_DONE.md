...
| Semantic Rebase runtime | Pure CRS policy tests exist, and the first durable spreadsheet stale-agent path now records conflict packets, dedupes rebased proposals, and applies approved proposals only through final CAS. | Extend CRS beyond stale single-cell agent writes: patch-bundle publish, draft conflicts, proposal-approval CAS conflicts, LLM resolver action, validators, richer semantic cards, and notebook/deck integration. | Conflicts over formulas, assumptions, evidence, and memo meaning route to deterministic merge, validated model proposal, human review, or rejection without bypassing CAS. |
...
| Public/private boundaries | Product has public room and private agent lanes; memory-mode Playwright covers the main no-leak browser path, and live broad Convex coverage now checks public/private chat isolation across two browser contexts. | Add live Convex browser coverage for no private file/artifact leakage into room trace, wiki, wall, public artifacts, exports, and downstream handoffs. | Privacy boundary failures block release. |
