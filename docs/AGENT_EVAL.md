...
- 🧾 **3-statement modeling test · Solve mode** — the private June 2026 batch is retained as historical research, but its 30-day provider-freshness window expired. It is not a current launch promotion; rerun the private five-run batch before restoring the live checkmark (`docs/eval/FINANCE_MODEL_EVAL.md`).
...
`docs/eval/finance-model-live.json` summary. That file records the June 2026
five-run research batch, but it is now historical and not a current launch
promotion. A new private `--runs=5 --record` batch plus a passing
`npm run proofs:staleness` is required before live full-solve language returns.
