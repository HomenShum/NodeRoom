...
| Omnigent / Omniagent bridge | `examples/omnigent/*.yaml` plus `src/nodeagent/skills/integration/omnigentAdapter.ts` and `npm run omnigent:nodeagent:smoke`. Omnigent stays the outer harness; NodeAgent remains the room-native runtime. | built at repo-smoke level; outer `omni run` requires installed Omnigent CLI |
