...
| Dedicated startup diligence walkthrough | Recaptured/rendered/media-judged | `startup-diligence-live-join` scripts a three-user CardioNova/runway join flow and is covered by Gemini judge run `20260614T233419Z` (`publish`, `10.9/16`); `startup-diligence-war-room` scripts CardioNova intake, five-company batch diligence, runway/milestone work, no-clobber proof, private banker lane, and draft-only downstream handoff and is covered by run `20260617T2015Z` (`publish`, `10.4/16`, no defects reported) | "Startup diligence walkthrough is aligned to the latest product target, with live shell proof plus deterministic agent-package proof." |
...
| Trace Lens review surface | Built and browser-smoked | `src/ui/traceLens/TraceLensPanel.tsx`, `src/ui/traceLens/useTraceLens.tsx`, `docs/demo/STARTUP_DILIGENCE_LATEST_REVIEW.md` | "Reviewers can inspect proof, runtime trace, and gated builder/code context from the surface." |
| Banker Coach OKF review surface | Built and browser-smoked | `src/ui/artifacts/BankerCoachPanel.tsx`, `src/ui/bankerCoachPacket.ts`, `docs/demo/STARTUP_DILIGENCE_LATEST_REVIEW.md` | "Banker Coach surfaces OKF/evidence context for readiness review and handoff." |
...
- `npm run media:gemini-judge -- --only startup-diligence --include-ignored`: live-join publish on 2026-06-14, run `20260614T233419Z`, score `10.9/16`, with one P2 presentation-polish note. Current war-room publish media judge is run `20260617T2015Z`, score `10.4/16`, with no defects reported.
...
- `npm run eval:startup-diligence:live`: pass on 2026-06-15, 9/9 checks; writes `docs/eval/startup-diligence-war-room-live-results.json` and updates `docs/eval/startup-diligence-war-room-live.json`. This is a Convex contract proof with `providerProducedContent: false`.
- `npm run eval:startup-diligence:provider`: pass on 2026-06-15, 9/9 checks; `gpt-5.4-nano` generated the CardioNova `CellPayload` and final copy, wrote `docs/eval/startup-diligence-provider-results.json`, and recorded route/cost/runtime evidence (`ms=2914`, `costUsd=0.00042775`).
