...
> do not quote them. Status vocabulary: [`docs/eval/CERTIFICATION_GATES.md`](docs/eval/CERTIFICATION_GATES.md)
> ("proven" = an official score was imported, never a leaderboard "pass").

<!-- proof-release:start -->
<!-- Generated by `npm run proofs:publish`; do not edit this block by hand. -->
## Proof Release: Methods, Results, And Receipts

**Certification status:** `certified` - all required receipts and the persisted gate pass

The current packet covers **1,739/1,739 staged official tasks** and keeps execution coverage separate from task performance. [Full methods, findings, fixes, versions, and limitations](docs/release/NODEROOM_PROOF_RELEASE.md).

| Lane | Executed coverage | Measured result | Receipt |
|---|---:|---|---|
| SpreadsheetBench V1 | 912/912 | 70/912 official pass; avg 0.096126; candidate proxy 89/912, avg 0.335084 | [JSON](docs/eval/spreadsheetbench-v1-accepted-official-scorer-receipt.json) |
| SpreadsheetBench Verified400 | 400/400 | 14/400 candidate proxy pass; avg 0.259266 | [JSON](docs/eval/spreadsheetbench-v1-verified-400-model-run.json) |
| SpreadsheetBench V2 | 321/321 | 0/321 official pass; avg 0; candidate proxy 0/321, avg 0.523337 | [JSON](docs/eval/spreadsheetbench-v2-accepted-official-scorer-receipt.json) |
| FinAuditing | 332/332 FinMR rows | FinRE macro F1: 0.162658; cost $0.041812 | [JSON](docs/eval/proofloop-official-scores/finauditing.json) |
| MBABench | 38/38 cases | Mean score: 11.513158; cost $6.67212 | [JSON](docs/eval/proofloop-official-scores/workstreambench.json) |
| Finch / FinWorkBench | 172/172 tasks | Mean score: 0.087209; cost $1.368019 | [JSON](docs/eval/proofloop-official-scores/finch.json) |
...
[Full proof video](episodes/noderoom-proof-release-v1/renders/short.mp4) | [Storyboard](episodes/noderoom-proof-release-v1/storyboard.yaml) | [Media manifest](episodes/noderoom-proof-release-v1/media-manifest.json) | [Visual judge: publish, 16/16](episodes/noderoom-proof-release-v1/judge.md)
...
<details><summary><b>Six-persona fresh-user dogfood: 6/6 passed, 0 console errors</b></summary>

| Analyst | Researcher | Finance operator |
| --- | --- | --- |
| ![Analyst fresh-user proof](docs/release/media/persona-analyst.png) | ![Researcher fresh-user proof](docs/release/media/persona-researcher.png) | ![Finance operator fresh-user proof](docs/release/media/persona-finance-operator.png) |

| Founder | Reviewer | Guest observer |
| --- | --- | --- |
| ![Founder fresh-user proof](docs/release/media/persona-founder.png) | ![Reviewer fresh-user proof](docs/release/media/persona-reviewer.png) | ![Guest observer fresh-user proof](docs/release/media/persona-guest-observer.png) |

[Machine-readable persona receipt](docs/eval/noderoom-persona-dogfood-receipt.json)
...
(`evals/financeModelLive.ts`; content-based leakage gate). The June private
live result is retained as historical evidence in
`docs/eval/finance-model-live.json`; current revalidation status is tracked
separately and controls whether any champion claim is active.

**The live scoreboard is the point, and it stays honest:** the June full-solve
result is now historical, not an active champion claim. On 2026-07-12 its
30-day freshness gate fired; a bounded revalidation of
`deepseek/deepseek-v4-flash` stopped at the 50-second model budget before any
workbook write (0/1, $0, no answer-key leakage, no clobber). The redacted failed
receipt is
[`finance-model-live-revalidation-2026-07-12.json`](docs/eval/finance-model-live-revalidation-2026-07-12.json).
The earlier 5/5 three-variant batch remains in
[`finance-model-live.json`](docs/eval/finance-model-live.json) as historical
evidence only. No full-solve route is currently promoted; the deterministic
synthetic lane and the previously measured free income rung remain scoped
engineering proofs, not a current private-workbook reliability claim.
...
|---|---|---|
| **1 - Do it for me** (autonomous solve) | variance/footnote/note/wall goldens - GTM research enrichment (v3 cheap/free smoke, 18/28 routes 9/9) - executable professional subset (GTM runtime enrichment, messy-sheet parsing, cross-file note write, grounded wiki update, finance reconciliation) - chat-first lead capture through live room tools - credit cascade + cell-mapping rejection - deterministic 3-statement solve harness; the June private full-lane result is historical after a failed July freshness revalidation | background chat-to-research intake - re-promote a private full-solve route through a fresh 5-run batch - SEC model-build flagship - N-doc research (benchmark v4) - file-drop ingestion (10-K/XLSX/receipts) - knowledge-organization pack |
