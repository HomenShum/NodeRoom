...
test.describe("chat — optimistic send + edit (memory mode)", () => {
  ...
  test("can send an artifact reference without extra text", async ({ page }) => {
    ...
    const bubble = chat.locator('[data-testid="chat-message"]:has(.r-msg-ref)').filter({ hasText: "Q3 variance" });
    await expect(bubble).toHaveCount(1);
    await expect(bubble).toBeVisible();
  ...
  test("@nodeagent quick chips replace /ask and /free as the taught public agent UX", async ({ page }) => {
    ...
    const suggestions = chat.locator(".r-composer-suggestions");
    const agentChips = suggestions.getByRole("button").filter({ hasText: /^@nodeagent / });
    await expect(agentChips.first()).toBeVisible();
    await expect(suggestions.getByRole("button").filter({ hasText: /^\/(ask|free)\b/ })).toHaveCount(0);
