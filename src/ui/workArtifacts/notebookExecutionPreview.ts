...
export type NotebookExecutionPreviewKind = Extract<NotebookTypedBlockKind, "calculation" | "sql" | "chart">;
...
export function buildNotebookExecutionPreview(structure: NotebookArtifactStructure): NotebookExecutionPreview {
  ...
  const executableKinds = new Set<NotebookTypedBlockKind>(["calculation", "sql", "chart"]);
  const items = typed
