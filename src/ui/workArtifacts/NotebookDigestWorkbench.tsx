      ...
      <div className="wa-notebook-grid">
        ...
        <aside className="wa-notebook-side">
          ...
          <div className="wa-notebook-side-card" data-testid="notebook-execution-preview">
            ...
            <div className="wa-notebook-kernel-modes" role="group" aria-label="Notebook kernel backend">
              <button type="button" data-selected={kernelBackend === "safe"} onClick={() => setKernelBackend("safe")}>Safe</button>
              <button type="button" data-selected={kernelBackend === "pyodide"} onClick={() => setKernelBackend("pyodide")}>Pyodide</button>
            </div>
              ...
              <div className="wa-notebook-exec-list">
                {executionPreview.items.slice(0, 8).map((item) => (
                  <div key={item.id} className="wa-notebook-exec-item" data-kind={item.kind} data-status={item.status} data-testid="notebook-execution-preview-item">
                    ...
                    {onExecuteKernel && (
                      <div className="wa-notebook-exec-actions">
                        <button type="button" className="wa-notebook-exec-run" data-testid="notebook-kernel-run" disabled={item.status !== "ready" || runningKernelId !== null || (kernelBackend === "pyodide") !== (item.kind === "python")} onClick={() => void executeKernel(item)}>
                          {runningKernelId === item.id ? <LoaderCircle size={11} className="wa-spin" /> : <Play size={11} />}
                          {runningKernelId === item.id ? "Running" : "Run"}
                        </button>
                        {runningKernelId === item.id && (
                          <button type="button" className="wa-notebook-exec-cancel" data-testid="notebook-kernel-cancel" title="Cancel kernel" aria-label="Cancel kernel" onClick={() => kernelAbortRef.current?.abort()}>
                            <Square size={10} />
                          </button>
                        )}
                      </div>
