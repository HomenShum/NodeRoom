...
export function ArtifactSheet({ ctx }: { ctx: MobileCtx }): React.ReactElement {
  ...
  const send = (txt?: string): void => {
    ...
    if (livePreview) {
      const request = [
        `Create a governed deck patch proposal for work artifact ${liveDeck?.workArtifactId ?? DECK.id}.`,
        `Slide ${slide.index} (${slide.id}): ${slide.title}.`,
        ...
        `Use only room source artifacts: ${liveDeck?.sourceIds.join(', ') || 'none attached'}.`,
