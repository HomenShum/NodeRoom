  ...
  return {
    ...
    commands: [
      ...
      },
      {
        ...
        purpose: "Install setup packs and enforce native launch, session export, and ProofLoop gate receipts for agent hosts.",
        writes: "repo-config",
        json: false,
        options: [
          "list",
          "setup all|codex|claude-code|cursor|windsurf|devin|generic-cli",
          "launch codex|claude-code|cursor|windsurf|devin|devin-cli|devin-api|generic-cli --prompt <file>",
          "collect <host> --run-dir <dir> --session <file>",
          "verify <host> --run-dir <dir> --strict",
          "--local",
          "--command <cmd>",
          "--strict",
        ],
        responseShape: ".proofloop/setup/agents/*.json setup packs plus native launch/session/verify receipts",
      },
      {
        id: "mcp",
        usage: "proofloop mcp serve",
        purpose: "Expose ProofLoop setup, repair, provider, memory, graph, dashboard, and gate commands as stdio MCP tools.",
        writes: "local-proof-state",
        json: false,
        options: ["serve"],
        responseShape: "JSON-RPC stdio MCP server with tools/list and tools/call",
      },
      {
        id: "supervise",
      ...
      },
      {
        ...
        writes: "local-proof-state",
        json: false,
        options: ["setup all|butterbase|neo4j|rocketride|daytona|cognee|nebius|opsera", "--strict"],
      ...
      {
        id: "brief",
        usage: "proofloop brief latest",
        purpose: "Write the chart/report pack and print a concise proof brief.",
        writes: "proof-artifacts",
        json: false,
        options: ["<runId|latest>", "--out-dir <path>"],
        responseShape: "chart/report artifact paths plus summary",
      },
      {
        id: "dashboard",
        usage: "proofloop dashboard export latest",
        purpose: "Write deterministic dashboard JSON/HTML proof export for a run.",
        writes: "proof-artifacts",
        json: false,
        options: ["export <runId|latest>", "screenshot <runId|latest>", "--out-dir <path>"],
        responseShape: ".proofloop/dashboard/<target>/dashboard.json and .html",
      },
      {
        id: "graph",
        usage: "proofloop graph ingest --backend neo4j",
        purpose: "Index local code graph and export Cypher for a bring-your-own Neo4j server.",
        writes: "local-proof-state",
        json: false,
        options: ["index", "ingest --backend neo4j", "query --failure latest", "search <query>", "blast-radius", "export-cypher"],
        responseShape: "code graph stats/search output/export path",
      },
      {
        id: "search",
        usage: "proofloop search --hybrid \"<query>\"",
        purpose: "Search code graph and compacted proof memory together.",
        writes: "none",
        json: false,
        options: ["--hybrid <query>", "<query>"],
        responseShape: "graph result section plus memory result section",
      },
      {
        id: "memory",
        ...
        writes: "local-proof-state",
        json: false,
        options: ["init", "compact latest", "seed-dogfood", "remember-run latest --backend cognee", "search <query>", "recall <query>", "show <id>", "doctor"],
  ...
  if (topic === "agents") {
    return {
      ...
      sections: [
        {
          heading: "Fast Path",
          body: ["Use the CLI as the agent control surface. Pull only the command, receipt, setup pack, or repair prompt needed for the next step."],
          commands: [
            ...
            "npm run proofloop -- agents launch devin-cli --prompt <repair-prompt.md>",
            "npm run proofloop -- agents collect devin-cli --run-dir <native-run-dir>",
            "npm run proofloop -- agents verify devin-cli --run-dir <native-run-dir> --strict",
...
function checkProofloopGithubWorkflow(root: string): ProofloopDoctorCheck {
  ...
  if (!existsSync(workflowDir)) {
    return {
      ...
      fix: "Run `npm run proofloop -- ci install github --goal official-scores` if GitHub Actions should gate this repo.",
    };
  }
  ...
  if (!proofWorkflow) {
    return {
      ...
      fix: "Run `npm run proofloop -- ci install github --goal official-scores`.",
    };
  }
  return {
    id: "proofloop-github-workflow",
