export type NotebookKernelKind = "calculation" | "sql" | "chart" | "python";
export type NotebookKernelStatus = "completed" | "blocked" | "failed" | "timed_out" | "cancelled";
...
export type NotebookKernelResult = {
  schema: 1;
  kernelVersion: "noderoom-safe-kernel-v1" | "noderoom-kernel-broker-v1";
  kind: NotebookKernelKind;
  ...
  receipt: {
    inputHash: string;
    outputHash: string;
    backend: "memory" | "convex" | "preview" | "safe" | "pyodide" | "jupyter" | "container";
    executedAt: number;
    rowCount: number;
    executionId?: string;
    durationMs?: number;
    networkPolicy?: "deny";
    isolation?: "in_process_bounded" | "web_worker" | "remote_jupyter" | "remote_container";
    traceId?: string;
    artifactIds?: string[];
    resourceLimits?: {
      timeoutMs: number;
...
export function executeNotebookKernel(request: NotebookKernelRequest, options: KernelOptions = {}): NotebookKernelResult {
  ...
  try {
    ...
    if (request.kind === "chart") return executeChart(input, tables, backend, executedAt);
    return result("python", input, "blocked", "Python requires the isolated Pyodide or an approved external kernel backend.", "python_requires_isolated_backend", backend, executedAt);
  } catch (error) {
