...
export type RoomArtifactProjectionCache = { roomId: string; fingerprints: readonly RoomArtifactFingerprint[]; snapshot: NodeBookSurfaceSnapshot };
...
export function updateRoomArtifactProjectionCache(cache: RoomArtifactProjectionCache | undefined, roomId: string, artifacts: readonly Artifact[]) {
  ...
  return { roomId, fingerprints: artifacts.map(fingerprint), snapshot: projectRoomArtifactsToNodeBook(roomId, artifacts) };
}

function NodeRoomNodeBookWorkspaceSurfaceInner({ roomId, onOpenArtifact, artifacts }: { roomId: string; onOpenArtifact(id: string): void; artifacts: readonly Artifact[] }) {
  const projection = useRef<RoomArtifactProjectionCache | undefined>(undefined);
  projection.current = updateRoomArtifactProjectionCache(projection.current, roomId, artifacts);
  const snapshot = projection.current.snapshot;
...
export function NodeRoomNodeBookWorkspaceSurfaceFromArtifacts(props: { roomId: string; onOpenArtifact(id: string): void; artifacts: readonly Artifact[] }) {
  ...
  const revision = roomArtifactProjectionRevision(props.roomId, artifacts);
  return <NodeBookHostErrorBoundary resetKey={`${props.roomId}:${revision}`}><NodeRoomNodeBookWorkspaceSurfaceInner {...props} /></NodeBookHostErrorBoundary>;
}
