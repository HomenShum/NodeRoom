...
describe("work artifact adapters", () => {
  ...
  it("round-trips a collaborative deck through an ordinary CAS-backed note artifact", () => {
    ...
    const storedStoryboard = input.seed.find((item) => item.id === "deck_storyboard")?.value as ReturnType<typeof normalizeCollaborativeDeck>;
    expect(snapshot).toMatchObject({ artifactId: "art-deck", elementVersion: 1 });
    expect(snapshot?.storyboard.planHash).toBe(storedStoryboard.planHash);
