...
describe("ProofLoop prod browser adapter ledger", () => {
  it("version-tracks every blocked prod proxy adapter family", () => {
    ...
    expect(ledger.harnessVersion).toBe("prod-browser-adapters-2026-07-05.3");
    expect(ledger.summary.adaptersTracked).toBe(6);
    expect(ledger.summary.contractScaffolded).toBe(2);
    expect(ledger.summary.browserScenarioMissing).toBe(2);
