...
describe("ProofLoop prod proxy benchmark matrix", () => {
  it("keeps the full task denominator instead of collapsing to the 3-task adapter smoke", () => {
    ...
    expect(report.summary.runnableProdBrowserTaskTargets).toBe(103);
    expect(report.summary.blockedTaskTargets).toBe(1251);
  });
  ...
  it("marks SpreadsheetBench full bundles as blocked until a generic prod browser workbook adapter exists", () => {
    const report = buildProofloopProdProxyBenchmarkMatrix();
    ...
    expect(v1?.blockedTasks).toBe(912);
    expect(v1?.tasks[0]?.blockers[0]).toContain("Generic SpreadsheetBench official workbook upload");
    expect(v2?.taskCount).toBe(321);
    expect(v2?.blockedTasks).toBe(321);
  });
