...
describe("ProofLoop standalone runner dogfood plan", () => {
  ...
  it("summarizes current unfinished proxy and benchmark work without dropping the denominator", () => {
    ...
    expect(plan.summary.adapterGapTasks).toBe(6);
    expect(plan.summary.guardedLiveRunBatchTasks).toBe(3);
  ...
  it("creates compact tasks for adapter repair, guarded live batches, and official score blockers", () => {
    ...
    expect(ids).toContain("adapter-gap.spreadsheetbench-v1-full-912");
    expect(ids).toContain("adapter-gap.spreadsheetbench-v2-full-321");
    ...
    const spreadsheet = plan.tasks.find((task) => task.id === "adapter-gap.spreadsheetbench-v1-full-912");
    expect(spreadsheet?.command).toBe("npm run benchmark:proofloop:prod-browser-adapters");
    expect(spreadsheet?.estimatedCostUsd).toBe(0);
    expect(spreadsheet?.paidModelRequired).toBe(false);
    expect(spreadsheet?.commands.some((command) => command.command.includes("prod-browser-adapters"))).toBe(true);
  });
