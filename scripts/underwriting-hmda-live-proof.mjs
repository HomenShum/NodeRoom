...
const OUT_DIR = resolve(process.env.UNDERWRITING_LIVE_OUTPUT_DIR ?? "test-results-underwriting-direct");
...
const HARNESS_VERSION = "hmda-underwriting-live-proof-v1.0.0";
const PROOF_CONTRACT_VERSION = "prod-live-hmda-underwriting-v1";
...
function ensurePacketReady() {
  ...
  const result = spawnSync(process.execPath, [resolve("scripts/underwriting-hmda-live-packet.mjs")], {
    cwd: resolve("."),
    encoding: "utf8",
  });
  if (result.status !== 0) {
...
function artifactTitlePattern(file) {
  const stem = basename(file).replace(/\.[^.]+$/, "").replace(/[_-]+/g, " ");
  return new RegExp(stem.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i");
}

async function waitVisible(locator, timeout = 30_000) {
...
async function openFreshLiveSheet(page) {
  ...
  if (page.url().includes("mode=memory")) throw new Error("underwriting proof must start from live landing, not memory mode");
...
async function ensureBinderOpen(page) {
  ...
  await waitVisible(leftRail, 30_000);
}

async function activateSheet1(page) {
  ...
  if (await sheetTab.isVisible().catch(() => false)) await sheetTab.click();
  await waitVisible(page.getByTestId("sheet-grid").first(), 30_000);
}
...
async function readPredictions(page) {
  return page.evaluate(() => {
    ...
    const visibleSheet = [...document.querySelectorAll('[data-testid="sheet-grid"]')]
    ...
    if (visibleSheet) {
      ...
      for (const [index, tr] of tableRows.entries()) {
        ...
        if (id || action || label || confidence || reason) {
          rows.push({ row: index + 1, application_id: id, predicted_action_taken: action, predicted_label: label, confidence, brief_reason: reason });
        }
      }
      return rows;
    }
    for (let i = 1; i <= 80; i += 1) {
      ...
      if (id || action || label || confidence || reason) {
        rows.push({ row: i, application_id: id, predicted_action_taken: action, predicted_label: label, confidence, brief_reason: reason });
      }
    }
    return rows;
  });
...
function scoreRows(rows, labels) {
  ...
  for (const row of rows) {
    ...
    if (predicted == null) unparseable += 1;
    else if (predicted === actual.action_taken) correct += 1;
    else incorrect += 1;
  ...
  return { n: labels.length, matchedRows: seen.size, attempted, correct, incorrect, unparseable, missing, accuracy, attemptedAccuracy, confusion, predictions };
}

function outputRowsComplete(score) {
...
function writeProof(path, proof) {
...
async function readBackendJobReceipt(roomCode) {
  if (!roomCode) return { ok: false, reason: "missing_room_code" };
  const convexUrl = process.env.UNDERWRITING_CONVEX_URL
    ?? process.env.VITE_CONVEX_URL
    ?? process.env.CONVEX_URL
    ?? "https://zealous-goshawk-766.convex.cloud";
  try {
    const client = new ConvexHttpClient(convexUrl);
    const receipt = await client.query(api.agentJobs.benchmarkJobReceipt, { roomCode });
    return { ok: true, convexUrl, ...receipt };
  } catch (error) {
    return {
      ok: false,
      reason: "convex_query_failed",
      convexUrl,
...
const answerKey = readJson(ANSWER_KEY);
const packetManifest = readJson(PACKET_MANIFEST);
const pageErrors = [];
const consoleErrors = [];
...
try {
  ...
  for (const file of [FEATURE_CSV, TASK_MD, SOURCE_MANIFEST_MD]) {
    await waitVisible(page.getByTestId("binder-artifact").filter({ hasText: artifactTitlePattern(file) }).first(), 45_000);
  }
  ...
  const chat = await publicChat(page);
  ...
  const screenshotPath = resolve(OUT_DIR, "underwriting-hmda-live-sheet.png");
  await page.screenshot({ path: screenshotPath, fullPage: false });
  const backend = await readBackendJobReceipt(roomCodeFromUrl(page.url()));
  const passed =
    jobStatusVisible
    && pageErrors.length === 0
  ...
  proof = {
    ...
    harness: {
      ...
      runner: "scripts/underwriting-hmda-live-proof.mjs",
      browserHarness: "playwright-chromium-direct-v1",
      backendReceiptQuery: "agentJobs:benchmarkJobReceipt",
      scorer: "withheld-local-answer-key-v1",
      packetBuilder: "scripts/underwriting-hmda-live-packet.mjs",
      outputColumns: OUTPUT_COLUMNS,
      requiredGates: [
        "fresh production room on https://noderoom.live",
        "memory mode disabled",
        "answer key remains local-only and is not uploaded",
        "visible Sheet 1 contains all output columns for all applications",
        "withheld-key score reaches required accuracy",
        "Convex backend job status is completed",
        "reasoning frame status is completed",
        "no page errors",
      ],
    },
    iterationLedger: {
      current: "I10-final-proofloop-contract",
      document: "docs/eval/UNDERWRITING_LIVE_PROOFLOOP.md",
    },
  ...
  writeProof(resolve(OUT_DIR, "underwriting-hmda-live-proof.json"), proof);
  if (!passed) throw new Error(`HMDA underwriting live proof failed; receipt: ${PROOF_PATH}`);
  console.log(JSON.stringify({
    passed,
    roomUrl: proof.roomUrl,
    jobStatus,
    backendStatus: backend.job?.status,
    matchedRows: score.matchedRows,
