 ...
 *
 * Run:  npx tsx scripts/walkthroughs/episode-cli.ts <episodeId> [--local-voiceover] [--skip-voiceover] [--skip-judge]
 *   or: npm run episode -- <episodeId> [--local-voiceover] [--skip-voiceover] [--skip-judge]
 *
 * --local-voiceover: use Windows SAPI locally instead of a provider (no TTS spend / no key needed).
...
if (!id) { console.error("usage: episode-cli.ts <episodeId> [--local-voiceover] [--skip-voiceover] [--skip-judge]"); process.exit(1); }

const run = (cmd: string) => { console.log(`\n$ ${cmd}`); execSync(cmd, { stdio: "inherit" }); };

if (!flags.includes("--skip-voiceover")) {
  run(`npx tsx scripts/walkthroughs/voiceover.ts ${id}${flags.includes("--local-voiceover") ? " --local" : ""}`);
}
...
run(`npx tsx scripts/walkthroughs/publish-episode.ts ${id}`);
