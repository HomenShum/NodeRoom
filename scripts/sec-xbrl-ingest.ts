 ...
 * tie-out facts of the latest 10-K to one (accn, current-period end) — handling
 * the dual current/prior balance sheet — then emits a CLEAN task packet plus
 ...
 *
 * Usage: tsx scripts/sec-xbrl-ingest.ts [CIK ...]
 *   default CIKs = a cross-sector large-cap set. Writes
...
/** Align every tie-out tag to the latest 10-K's current period (one accn, one end). */
function alignLatest10K(facts: any, cik: string, name: string): CompanyXbrlFacts & { accn: string; period: { instantEnd: string; durationStart: string | null } } {
  const assets = rowsFor(facts, "Assets").filter((f) => f.form === "10-K");
  if (assets.length === 0) throw new Error(`${name}: no 10-K Assets`);
  const accn = [...assets].sort((a, b) => (a.filed < b.filed ? 1 : -1))[0].accn;
  ...
  return { cik, name, accn, facts: out, period: { instantEnd, durationStart } };
}
...
async function main() {
  ...
  const cleanCompanies: Array<CompanyXbrlFacts & { accn: string; period: unknown }> = [];
  ...
  for (const t of targets) {
    try {
      ...
      const aligned = alignLatest10K(facts, t.cik, t.name);
      ...
      console.log(`${t.name}: accn ${aligned.accn} end ${aligned.period && (aligned.period as any).instantEnd} · ${applicable} identities applicable · clean violations ${cleanTruth.length}`);
  ...
  writeFileSync(resolve(outDir, "benchmark.json"), JSON.stringify({ source: "SEC EDGAR companyfacts (us-gaap), latest 10-K, (accn,end)-aligned", officialScoreClaim: false, generatedFor: "deterministic tie-out audit", items: dataset }, null, 2));
  writeFileSync(resolve(outDir, "fixtures.json"), JSON.stringify({ source: "SEC EDGAR companyfacts API (us-gaap), latest 10-K, (accn,end)-aligned", companies: cleanCompanies.map(stripMeta) }, null, 2));
